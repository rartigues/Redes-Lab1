...
def recvFile(Emo):
    ...
    repeticiones = int((int(size) / 1024)+1)
    porcentaje=0
    # Recibindo la informacion del archivo del cliente
    parts = []
    cont=0
    Emo.send(str(repeticiones).encode(FORMAT))
    
    with open(name, "wb") as file:
        while True:
            data = Emo.recv(1024)          
            #Emo.send("[SERVER] Informacion del archivo recibida.\n".encode(FORMAT))
            if not data:
                break
            parts.append(data)
            progressBarr(len(parts)/repeticiones*100)
            #create an if statement to check each time if the percentage increases 1%
            '''
            if(int((len(parts)/repeticiones)*100)==porcentaje):
                #progressBarr(porcentaje)
                print(f"[RECV] {porcentaje}%")
                Emo.send(str(porcentaje).encode(FORMAT))
                porcentaje+=1
            '''
            #print the percentage of the file received
...
def sendFile(client):
    ...
    with open("ejemplo/funcionaxd.png","rb") as file:            
...
def main():
    ...
    while True:
        ...
        answer=Emo.recv(1024).decode(FORMAT)
        if(answer=="1"):
            recvFile(Emo)
        elif(answer=="2"):
            sendFile(Emo)
